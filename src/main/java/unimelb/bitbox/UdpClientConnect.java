...
/*
 * process the client connection in the udp mode.
 */

public class UdpClientConnect extends Thread implements Runnable
{
	private static Logger log = Logger.getLogger(UdpClientConnect.class.getName());
	ArrayList<HostPort> peers;
	ServerMain2 servermain2;

	public UdpClientConnect(ArrayList<HostPort> peers, ServerMain2 servermain2)
	{
		super();
		this.peers = peers;
			...
			while (true)
			{
				...
				if (command.equals("AUTH_REQUEST"))
				{
					...
					if (messagejson.get("identity").toString().equals(id))
					{
						...
						new HandleClient2(clientSocket, peers, servermain2, key).start();
					}
					else
					{
						AUTH_RESPONSE.put("command", "AUTH_RESPONSE");
